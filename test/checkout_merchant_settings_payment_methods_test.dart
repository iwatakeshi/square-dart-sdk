//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:square_dart_sdk/api.dart';
import 'package:test/test.dart';

// tests for CheckoutMerchantSettingsPaymentMethods
void main() {
  // final instance = CheckoutMerchantSettingsPaymentMethods();

  group('test CheckoutMerchantSettingsPaymentMethods', () {
    // PaymentMethod applePay
    test('to test the property `applePay`', () async {
      // TODO
    });

    // PaymentMethod googlePay
    test('to test the property `googlePay`', () async {
      // TODO
    });

    // PaymentMethod cashApp
    test('to test the property `cashApp`', () async {
      // TODO
    });

    // AfterpayClearpay afterpayClearpay
    test('to test the property `afterpayClearpay`', () async {
      // TODO
    });
  });
}
