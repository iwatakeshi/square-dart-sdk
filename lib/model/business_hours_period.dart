...
class BusinessHoursPeriod {
  ...
  @override
  String toString() => 'BusinessHoursPeriod[dayOfWeek=$dayOfWeek, startLocalTime=$startLocalTime, endLocalTime=$endLocalTime]';
  ...
  /// Returns a new [BusinessHoursPeriod] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static BusinessHoursPeriod? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "BusinessHoursPeriod[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "BusinessHoursPeriod[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<BusinessHoursPeriod> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <BusinessHoursPeriod>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = BusinessHoursPeriod.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, BusinessHoursPeriod> mapFromJson(dynamic json) {
    final map = <String, BusinessHoursPeriod>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BusinessHoursPeriod.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of BusinessHoursPeriod-objects as value to a dart map
  static Map<String, List<BusinessHoursPeriod>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<BusinessHoursPeriod>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = BusinessHoursPeriod.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
